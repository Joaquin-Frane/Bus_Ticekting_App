...
import { db } from '../firebaseconfig';  // Import Firestore from your firebaseconfig.js
import { collection, getDocs, query, where } from "firebase/firestore";
import { Picker } from '@react-native-picker/picker';  // Correct Picker import
import { AntDesign } from '@expo/vector-icons'; // Icon for back button
import Toast from 'react-native-toast-message';

const TicketBookingScreen = ({ navigation, route }) => {
  const { predefinedRoute, predefinedDate } = route.params || {};  // Get the passed route params
  const [routes, setRoutes] = useState([]);  
  const [schedules, setSchedules] = useState([]);
  const [selectedDate, setSelectedDate] = useState(predefinedDate || '');  // Set the date if passed
  const [selectedRoute, setSelectedRoute] = useState(predefinedRoute || '');  // Set the route if passed
  ...
  const fetchSchedules = async (route, date) => {
    //console.log('Fetching schedules for:', route, date);
    const route1 = route.toString();
    try {
      const q = query(
      ...
      //console.log('Query result (raw):', scheduleSnapshot.docs);
      ...
      //console.log('Mapped schedule list:', scheduleList);  // Check if any schedules are returned
  ...
  const onChange = (event, selectedDate) => {
    ...
    setShow(false);
    setDate(currentDate);
    setSelectedDate(currentDate.toISOString().split('T')[0]);  // Format date for query
  };
      ...
      <ScrollView  showsVerticalScrollIndicator={false}>
        {schedules.map((schedule) => (
          <TouchableOpacity
            ...
            onPress={() => handlePress(schedule.id)} // Call handlePress with schedule.id
          >
